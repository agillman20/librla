...
randomized SVD for a data matrix taken from an experiment in (Tropp et
al. 2019). The results were generated by running the
\texttt{test\_sst\_mode} code in the \emph{climate\_analysis} folder
...
\begin{figure}
\centering
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio,alt={Illustration of the use of randomized factorization for image compression: (a) original image, and rank 30 approximations using (b) the randomized SVD, (c) the interpolatory decomposition, (d) the randomized SVD with 15 extra samples and 2 power iterations and (e) the interpolatory decomposition with 15 extra samples and 2 power iterations. The example is taken from (Duersch and Gu 2020). The original image is ``Silver Metal Round Gears'' from Pexels (Creative Commons Zero license). }]{imageEX.png}
