...
useful routines for solving total least squares problems and rank
deficient least squares problems, doing matrix approximation, and
...
our repository. The results show that the performance of \texttt{librla}
...
three different factorizations. The factorization speeds of all three
...
pivoted QR factorization of \({\bf A}\Omega\). The magnitude of the
diagonal entries of \({\bf R}\) are stored in a vector \({\bf diagR}\)
and are used in the stopping criterion for the tolerance option of the
factorization. \texttt{orth\_sketch} returns \({\bf Q}\) or a submatrix
of \({\bf Q}\), \({\bf diagR}\) and an error flag to the factorization
routine that called it. The matrix \({\bf Q}\) is then used to create
