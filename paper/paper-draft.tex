...
library is not available. The need is even greater for those using
high-level languages where standard coding techniques can result in
...
\(n \times block\_size\) whose entries are uniformly randomly sampled
from \([-1,1]\). The oversampling vectors are included in
\(block\_size\). In the tolerance option, the \(block\_size\) is
adaptively increased to ensure the range is sufficiently sampled and
includes the number of user specified extra samples. In the rank option,
...
factorization. If the sketching step terminates early, the factorization
routine falls back to a deterministic algorithm.
