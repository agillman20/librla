...
useful tools for solving total least squares problems, rank deficient
least squares problem, doing matrix approximation, and skeletonizing
(i.e.~subset selection) a matrix (Chan and Hansen 1992). \texttt{librla}
provides low rank QR factorizations, SVDs and interpolatory
decompositions written natively in Python, Julia and Matlab. The
...
possible. \texttt{librla} is designed for small to mid-range sized
matrices (i.e.~up to roughly 10,000 in size depending on computing
resources). \texttt{librla} is not intended for matrices that are larger
or need to be read from disk.
...
The user has the choice of specifying a tolerance or a desired rank. For
each method, the user has the option to use extra samples and to use a
specified number of power iterations. The package does include the
option to create low rank factorizations of matrices that are applied
via matrix vector multiplication codes. In order to use this option,
...
there is a large collection of related work: (Duersch and Gu 2020, 2017;
Mahoney and Drineas 2009; Meier and Nakatsukasa 2024; Martinsson et al.
2019, 2019; Sorensen and Embree 2016; Gu and Eisenstat 1996; Frieze et
al. 2004; Drineas, Kannan, et al. 2006a, 2006b; Drineas, Mahoney, et al.
2006, 2008; Sarlos 2006; Drineas et al. 2007, 2012; Rokhlin and Tygert
2008; Avron et al. 2010; Meng et al. 2014; Martinsson and Tropp 2020;
Roosta-Khorasani and Mahoney 2019; Pilanci and Wainwright 2017; Yao et
al. 2021; Mahoney 2011; Woodruff 2014; Drineas and Mahoney 2016, 2017;
Yang et al. 2015; Pilanci and Wainwright 2016; Erichson et al. 2018;
Voronin and Martinsson 2017; Balu and Furon 2016; Cormode et al. 2018;
Choi et al. 2020; Yu et al. 2017; Yu et al. 2018; Tropp et al. 2017;
Bjarkason 2019; Chowdhury et al. 2020; Lindquist et al. 2020; Feng et
al. 2018). The user specified rank algorithm in \texttt{librla} is
influenced by the \texttt{svd\_lowrank} routine in PyTorch.

\section{Statement of need}\label{statement-of-need}

While there is a large amount of research activity in the field of
randomized linear algebra, an easy to use and stable factorization
library is not available. The need is even greater for those using high
level languages where standard coding techniques can result in an
unnecessarily slow code. The research area of fast direct solvers
provides an example of the need for \texttt{librla}. For some time, the
development and use of fast direct solvers in Matlab required a
specialized wrapper on the Fortran package (Martinsson et al. 2008)
which is based on (Liberty et al. 2007). The lack of portability of the
wrapper has slowed the design and use of these solvers.

Currently, there are two related routines available in widely available,
...
the repository. The results show that the performance of \texttt{librla}
is comparable to that of PyTorch's \texttt{svd\_lowrank} and is faster
than the interpolatory decomposition \texttt{id\_decomp}. The library
\texttt{librla} is the only package that provides the user the option of
three different factorizations. The factorization speeds of all the
methods are comparable to PyTorch's \texttt{svd\_lowrank}.
...
\({rtol}\). The user has the option to use power iteration and specify
the number of iterations \(power\_iter\) to accelerate convergence.
Users are also provided the option to specify a number of oversampling
vectors. Roughly speaking, given a linear operator \({\bf A}\), desired
rank or stopping condition \(rtol\), specified \(block\_size\) and
\(power\_iter\), \texttt{orth\_sketch} randomly samples the range of
\({\bf A}\) to create an orthogonal basis for the range. The range is
sampled by applying \({\bf A}\) to a matrix \({ \Omega }\) of size
\(m \times block\_size\) columns whose entries are uniformly random
sampled from \([-1,1]\). The oversampling vectors are included in
\(block\_size\). In the tolerance option, the \(block\_size\) is
increased to ensure the range is sufficiently sampled and includes the
number of user specified extra samples. In the rank option, the size of
\(\Omega\) is determined by the desired rank and the number of user
specified extra samples. The orthogonal basis is formed by taking a
pivoted QR factorization of the \({\bf A \ \Omega}\). The magnitude of
the diagonal entries of \({\bf R}\) are stored in a vector
\({\bf diagR}\) and are used in the stopping criterion for the tolerance
option of the factorization. \texttt{orth\_sketch} returns \({\bf Q}\)
or a submatrix of \({\bf Q}\), \({\bf diagR}\) and an error flag to the
factorization routine that called it. The matrix \({\bf Q}\) is then
used to create the desired factorization.
...
\section{Demo codes}\label{demo-codes}
...
one factorization technique.

\section{Examples from the
literature}\label{examples-from-the-literature}

To illustrate the ability of the library handle problems of interest,
...
Examples similar to this are found throughout the randomized linear
algebra literature.
...
Figure \ref{fig:sst_modes} shows the first five randomized SVD modes
(empirical orthogonal function (EOF) spatial patterns and principal
component (PC) time series) of the NOAA ERSST v5 data.
...
seasonal variations, and EOF5 captures the ENSO signal.
\label{fig:sst_modes}}
\end{figure}

The image compression example \texttt{test\_image\_id} code in the
\emph{image\_analysis} folder produces 6 images. Figure
\ref{fig:image_orig} illustrates 5 of them: (a) the original image and
rank 30 approximations of the image using (b) the randomized SVD, (c)
the interpolatory decomposition, (d) the randomized SVD with 15 extra
samples and 2 power iterations and (e) the interpolatory decomposition
with 15 extra samples and 2 power iterations. While the rank of the
approximations is the same, the SVD produces an image with less
smearing. The examples with oversampling and power iterations further
highlight this result.

\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio,alt={Illustration of the use of randomized factorization for image compression: (a) original image, and rank 30 approximations using (b) the randomized SVD, (c) the interpolatory decomposition, (d) the randomized SVD with 15 extra samples and 2 power iterations and (e) the interpolatory decomposition with 15 extra samples and 2 power iterations. The example is taken from (Duersch and Gu 2020). The original image is ``Silver Metal Round Gears'' from Pexels (Creative Commons Zero license). }]{imageEX.png}
...
Metal Round Gears''} from Pexels (Creative Commons Zero license).
\label{fig:image_orig}}
\end{figure}
...
\bibitem[\citeproctext]{ref-Yu:2018}
Yu, W., Y. Gu, and Y. Li. 2018. {``Efficient Randomized Algorithms for
the Fixed-Precision Low-Rank Matrix Approximation.''} \emph{SIAM Journal
on Matrix Analysis and Applications} 39 (3): 1339--59.
