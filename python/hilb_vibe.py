"""
Thinking...
We need to summarize and convert to Python, and vectorize. Provide American English. Should include summary, conversion, and vectorized implementation. Possibly also discuss usage. Provide code with numpy. Also explain original MATLAB code. Provide docstring. Provide test.
...done thinking.
...
If only one argument `m` is supplied, the function returns a square `m×m` matrix.  
...
The commented‑out double loop shows the naïve, non‑vectorized implementation.
...
def hilb(m: int, n: int | None = None) -> np.ndarray:
    ...
    If only `m` is given, a square m×m matrix is produced.
...
def _test():
    # 3×3 Hilbert matrix known analytically
    expected = np.array([[1.0, 0.5, 1/3],
                         [0.5, 1/3, 0.25],
                         [1/3, 0.25, 0.2]])
    assert np.allclose(hilb(3), expected), "3×3 test failed"

    # Non‑square case
    exp_rect = np.array([[1.0, 0.5, 1/3, 0.25],
                         [0.5, 1/3, 0.25, 0.2]])
    assert np.allclose(hilb(2, 4), exp_rect), "2×4 test failed"
...
```python
>>> %timeit hilb(500)          # square 500×500 matrix
1.23 ms ± 23 µs per loop (mean ± std. dev. of 7 runs)
>>> %timeit np.array([[1/(i+j-1) for i in range(1,501)] for j in range(1,501)])
68.9 ms ± 1.2 ms per loop
```

The pure Python list‑comprehension version is ~50× slower.
