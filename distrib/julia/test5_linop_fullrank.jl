...
method parameter:
1. Dense matrix (baseline) - with method="lstsq"
...
function test_linop_fullrank()
    ...
    t1 = @elapsed k1, piv1, T1 = id_sketch(A, Float64(k_target), method="lstsq")
    ...
    @printf("  Time:      %.4f s\n", t1)
    if err1 < 1.0
        println("  [OK] Error < 1.0 (method=\"lstsq\" guarantees this)")
    end
    ...
    t2 = @elapsed k2, piv2, T2 = id_sketch(A_linop_explicit, Float64(k_target), method="lstsq")
    ...
    println("  Mode: Rank mode (rtol >= 1), method=\"lstsq\"")
    @printf("  Note: Extracts all %d columns via unit vectors (n matvecs)\n", n)

    t3 = @elapsed k3, piv3, T3 = id_sketch(A_linop_mf, Float64(k_target), method="lstsq")
    ...
    if k3 == k_target && err3 < 1.0
        @printf("  [OK] Matrix-free (method=\"lstsq\"): rank k=%d, error < 1.0\n", k_target)
    elseif err3 < 1.0
        println("  [OK] Error < 1.0 guaranteed by method=\"lstsq\"")
    end
    ...
    t4 = @elapsed k4, piv4, T4 = id_sketch(A_linop_mf, Float64(k_target), method="fast")
    ...
    @printf("  Speedup:   %.1fx faster than method=\"lstsq\"\n", speedup)
    ...
    println("-"^70)
    @printf("  Dense (method=\"lstsq\")        %4d    %.3e    %.3e    %.4fs\n", k1, err1, maxT1, t1)
    @printf("  Explicit LinOp (lstsq)       %4d    %.3e    %.3e    %.4fs\n", k2, err2, maxT2, t2)
    @printf("  Matrix-free (lstsq)          %4d    %.3e    %.3e    %.4fs\n", k3, err3, maxT3, t3)
    @printf("  Matrix-free (fast)           %4d    %.3e    %.3e    %.4fs\n", k4, err4, maxT4, t4)
    println("="^70)
    ...
    if success
        ...
        println("       method=\"lstsq\" guarantees error < 1.0 for all modes")
        @printf("       method=\"fast\" provides %.1fx speedup with acceptable error increase\n", speedup)
    end
