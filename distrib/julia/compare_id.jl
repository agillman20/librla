...
# - LibIDSketch:  Randomized QR sketching (default, recommended)
# - LibIDRRQR:    Deterministic RRQR via LAPACK geqp3
#
# Compares on metrics:
...
function compare_on_matrix(A::Matrix{T}, rtol_or_rank::Float64, name::String) where T
    ...
    # -------------------------------------------------------------------------
    # 1. LibIDSketch (randomized QR sketching)
    # -------------------------------------------------------------------------
    println("\n--- LibIDSketch.id_sketch (randomized QR) ---")

    t_sketch = @elapsed k_sketch, piv_sketch, T_sketch = id_sketch(A, rtol=rtol_or_rank)
    ...
    if err_sketch > 1.0
        ...
        println("  Recomputing with recompute_T=true for accurate lstsq-based T...")

        # Retry with recompute_T=true for accurate T computation via lstsq
        t_sketch = @elapsed k_sketch, piv_sketch, T_sketch = id_sketch(A, rtol=rtol_or_rank, recompute_T=true)
        ...
        @printf("  -> Recomputed: error = %.3e (recompute_T=true)\n", err_sketch)

        if err_sketch > 1.0
            error("[ERROR] Error still > 1.0 even with recompute_T=true!\n   Error = $err_sketch, Test: $name, rtol_or_rank=$rtol_or_rank")
        end
    end
    ...
    # -------------------------------------------------------------------------
    # 2. LibIDRRQR (deterministic RRQR)
    # -------------------------------------------------------------------------
    println("\n--- LibIDRRQR.id_rrqr (deterministic RRQR) ---")

    t_rrqr = @elapsed k_rrqr, piv_rrqr, T_rrqr = id_rrqr(A, rtol=rtol_or_rank)
...
function print_summary(results::Vector{ComparisonResult})
    ...
    @printf("  id_rrqr:     mean=%.3fs, min=%.3fs, max=%.3fs\n",
...
function main()
    ...
    println("LibIDSketch.id_sketch vs LibIDRRQR.id_rrqr")
    println("="^70)
    println("\nEnvironment:")
