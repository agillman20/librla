...
function test_svd_hilbert()
    ...
    # -------------------------------------------------------------------------
    println("\n1. LibIDSketch.svd_sketch (randomized SVD via sketching)")
    println("-"^70)

    t1 = @elapsed U1, s1, Vh1 = svd_sketch(A, rtol=Float64(k_target))
