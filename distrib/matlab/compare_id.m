% compare_id.m - Compare libid interpolative decomposition implementations
%
...
function compare_id()
    ...
    fprintf('libid.id_sketch vs libid_rrqr.id_rrqr\n');
    fprintf('======================================================================\n');
    fprintf('\nEnvironment:\n');
    if is_octave
        fprintf('  OCTAVE:     %s\n', version);
...
function result = compare_on_matrix(A, rtol_or_rank, name)
    ...
    % -------------------------------------------------------------------------
    % 1. libid.id_sketch (randomized QR sketching)
    % -------------------------------------------------------------------------
    fprintf('\n--- libid.id_sketch (randomized QR) ---\n');

    tic;
    [k_sketch, piv_sketch, T_sketch] = libid.id_sketch(A, rtol_or_rank);
    t_sketch = toc;
    ...
    if err_sketch > 1.0
        ...
        fprintf('  Recomputing with recomputeT=true for accurate lstsq-based T...\n');

        % Retry with recomputeT=true for accurate T computation via lstsq
        tic;
        [k_sketch, piv_sketch, T_sketch] = libid.id_sketch(A, rtol_or_rank, [], [], [], true);
        t_sketch = toc;
        ...
        fprintf('  -> Recomputed: error = %.3e (recomputeT=true)\n', err_sketch);

        if err_sketch > 1.0
            error('[ERROR] Error still > 1.0 even with recomputeT=true!\n   Error = %.6f, Test: %s, rtol_or_rank=%g', ...
    ...
    % -------------------------------------------------------------------------
    % 2. libid_rrqr.id_rrqr (deterministic RRQR)
    % -------------------------------------------------------------------------
    fprintf('\n--- libid_rrqr.id_rrqr (deterministic RRQR) ---\n');

    tic;
    [k_rrqr, piv_rrqr, T_rrqr] = libid_rrqr.id_rrqr(A, rtol_or_rank);
    t_rrqr = toc;
    ...
    % -------------------------------------------------------------------------
    fprintf('\n--- Summary ---\n');
    fprintf('%-22s %-8s %-12s %-12s %-10s\n', 'Method', 'Rank', 'Error', 'max|T|', 'Time (s)');
    fprintf('------------------------------------------------------------------------\n');
    fprintf('%-22s %-8d %-12.3e %-12.3e %-10.4f\n', 'libid (QR sketch)', k_sketch, err_sketch, max_T_sketch, t_sketch);
    fprintf('%-22s %-8d %-12.3e %-12.3e %-10.4f\n', 'RRQR (geqp3)', k_rrqr, err_rrqr, max_T_rrqr, t_rrqr);
...
function print_summary(results)
    ...
    fprintf('\nTiming Statistics:\n');
    fprintf('  id_sketch:   mean=%.3fs, min=%.3fs, max=%.3fs\n', ...
            mean(t_sketch_all), min(t_sketch_all), max(t_sketch_all));
    fprintf('  id_rrqr:     mean=%.3fs, min=%.3fs, max=%.3fs\n', ...
