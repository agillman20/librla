...
%
%   Tests librla.id_sketch with LinearOperators on a full-rank random matrix,
%   demonstrating method parameter:
%   1. Dense matrix (baseline) - with method='lstsq' (default)
...
function test5_linop_fullrank()
    ...
    tic;
    [k1, piv1, T1] = librla.id_sketch(A, k_target, 'method', 'lstsq');
    t1 = toc;
    ...
    fprintf('  Time:      %.4f s\n', t1);
    if err1 < 1.0
        fprintf('  [OK] Error < 1.0 (method=''lstsq'' guarantees this)\n');
    end
    ...
    tic;
    [k2, piv2, T2] = librla.id_sketch(A_linop_explicit, k_target, 'method', 'lstsq');
    t2 = toc;
    ...
    fprintf('  Mode: Rank mode (rtol >= 1), method=''lstsq''\n');
    fprintf('  Note: Extracts all %d columns via unit vectors (n matvecs)\n', n);

    tic;
    [k3, piv3, T3] = librla.id_sketch(A_linop_mf, k_target, 'method', 'lstsq');
    t3 = toc;
    ...
    if k3 == k_target && err3 < 1.0
        fprintf('  [OK] Matrix-free (method=''lstsq''): rank k=%d, error < 1.0\n', k_target);
    elseif err3 < 1.0
        fprintf('  [OK] Error < 1.0 guaranteed by method=''lstsq''\n');
    end
    ...
    fprintf('  Operator: %d x %d\n', A_linop_mf.m, A_linop_mf.n);
    fprintf('  Matrix-free: %d\n', isempty(A_linop_mf.matrix));
    fprintf('  Mode: Rank mode (rtol >= 1), method=''fast''\n');
    fprintf('  Note: Uses R matrix from sketch (fast approach, no extra matvecs)\n');

    tic;
    [k4, piv4, T4] = librla.id_sketch(A_linop_mf, k_target, 'method', 'fast');
    t4 = toc;
    ...
    fprintf('  Speedup:   %.1fx faster than method=''lstsq''\n', speedup);
    fprintf('  Error ratio: %.2fx (err_fast / err_lstsq)\n', error_ratio);

    if err4 > 1.0
        fprintf('  [NOTE] Error > 1.0 is expected for full-rank matrices with method=''fast''\n');
        fprintf('         This uses fast R-matrix approach, trading accuracy for speed\n');
    else
    ...
    fprintf('----------------------------------------------------------------------\n');
    fprintf('  Dense (method=''lstsq'')        %4d    %.3e    %.3e    %.4fs\n', k1, err1, maxT1, t1);
    fprintf('  Explicit LinOp (method=lstsq) %4d    %.3e    %.3e    %.4fs\n', k2, err2, maxT2, t2);
    fprintf('  Matrix-free (method=''lstsq'')  %4d    %.3e    %.3e    %.4fs\n', k3, err3, maxT3, t3);
    fprintf('  Matrix-free (method=''fast'')   %4d    %.3e    %.3e    %.4fs\n', k4, err4, maxT4, t4);
    fprintf('======================================================================\n');
    ...
    if success
        ...
        fprintf('       method=''lstsq'' guarantees error < 1.0 for all modes\n');
        fprintf('       method=''fast'' provides %.1fx speedup with acceptable error increase\n', speedup);
    end
end
