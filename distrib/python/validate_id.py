...
def compare_on_matrix(A, rtol_or_rank, name):
    ...
    if err_sketch > 1.0:
        ...
        print("  Recomputing with method='lstsq' for accurate lstsq-based T...")

        # Retry with method='lstsq' for accurate T computation via lstsq
        t0 = time.perf_counter()
        k_sketch, piv_sketch, T_sketch = id_sketch(A, rtol=float(rtol_or_rank), method='lstsq')
        t_sketch = time.perf_counter() - t0
        ...
        print(f"  -> Recomputed: error = {err_sketch:.3e} (method='lstsq')")

        if err_sketch > 1.0:
            raise ValueError(f"[ERROR] Error still > 1.0 even with method='lstsq'!\n"
    ...
    t0 = time.perf_counter()
    k_rrqr, piv_rrqr, T_rrqr = id_qrpiv(A, rtol=float(rtol_or_rank))
    ...
    print("-"*75)
    print(f"{'id_sketch (randomized)':<25s} {k_sketch:<8d} {err_sketch:<12.3e} {maxT_sketch:<12.3e} {t_sketch:<10.4f}")
    print(f"{'id_qrpiv (deterministic)':<25s} {k_rrqr:<8d} {err_rrqr:<12.3e} {maxT_rrqr:<12.3e} {t_rrqr:<10.4f}")
...
def main():
    ...
    print()
    print(f"{'Method':<25s} {'Avg Time':<12s} {'vs QR':<15s}")
    print("-"*80)
    print(f"{'id_sketch (randomized)':<25s} {avg_time_sketch:>8.4f}s    {avg_time_rrqr/avg_time_sketch:>6.1f}x ")
    print(f"{'id_qrpiv (deterministic)':<25s} {avg_time_rrqr:>8.4f}s    {1.0:>6.1f}x -")
    ...
    print(f"  id_sketch (randomized):   mean={avg_maxT_sketch:.3e}, min={min_maxT_sketch:.3e}, max={max_maxT_sketch:.3e}")
    print(f"  id_qrpiv (deterministic): mean={avg_maxT_rrqr:.3e}, min={min_maxT_rrqr:.3e}, max={max_maxT_rrqr:.3e}")
