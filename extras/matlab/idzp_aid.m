function [proj,krank,list] = idzp_aid(a, tol)
%
...
list = zeros(1,n);
krank = zeros(1,1);
proj = complex(zeros(2*n*(2*n2+1)+n2+1,1));

mex_id_ = 'idzpaid(c i double[], c i int[x], c i int[x], c io dcomplex[], c io double[], c io int[x], c io int[], c io dcomplex[])';
