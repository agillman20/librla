...
read_cifar.py
=============
Load the CIFAR-10 dataset from Python pickle files and return as NumPy arrays.
...
train_images : uint8 array, shape (32, 32, 3, 50000)
    Training images; each slice ``train_images[:,:,:,i]`` is a 32×32×3 RGB image.
test_labels  : uint8 array, shape (10000,)
    Test labels (values 0‑9).
test_images  : uint8 array, shape (32, 32, 3, 10000)
    Test images; each slice ``test_images[:,:,:,i]`` is a 32×32×3 RGB image.
...
# --------------------------------------------------------------
plt.figure()
# Images are stored in column‑wise order (MATLAB style).
# Transpose spatial dimensions for standard row‑major display.
plt.imshow(train_img[:, :, :, 0].transpose(1, 0, 2))
...
# --------------------------------------------------------------
plt.figure()
plt.imshow(test_img[:, :, :, 0].transpose(1, 0, 2))
...
def read_cifar(
    ...
    train_images = _reshape_cifar_images(train_data)
    print(f"Training set: {train_images.shape[3]} images total")
    ...
    test_images = _reshape_cifar_images(test_batch["data"])
    print(f"Test set: {test_images.shape[3]} images")

    # ---------- Summary ----------
    print(f"\nCIFAR-10 dataset loaded successfully:")
    print(f"  Training: {train_images.shape[3]} images, {len(train_labels)} labels")
    print(f"  Test:     {test_images.shape[3]} images, {len(test_labels)} labels")
...
def _reshape_cifar_images(data: np.ndarray) -> np.ndarray:
    """
    Convert (N×3072) uint8 array to (32×32×3×N).
    ...
    Returns
    -------
    images : np.ndarray, shape (32, 32, 3, N), dtype uint8
        Reshaped images with dimensions [height, width, channels, count].
    ...
    R_img = R.reshape(N, 32, 32)
    G_img = G.reshape(N, 32, 32)
    B_img = B.reshape(N, 32, 32)

    # Stack channels and transpose to (32, 32, 3, N)
    # We use column-major ordering (Fortran order) to match MATLAB
    images = np.zeros((32, 32, 3, N), dtype=np.uint8, order="F")
    for i in range(N):
        images[:, :, 0, i] = R_img[i].T  # Transpose for correct orientation
        images[:, :, 1, i] = G_img[i].T
        images[:, :, 2, i] = B_img[i].T
