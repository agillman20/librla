c
c Simple Fortan90 program that tests ZGEQP3RK
c  A = Q R
c
        implicit real *8 (a-h,o-z)

c Define the floating point kind to be  double_precision
        integer, parameter :: fp_kind = kind(0.0d0) 

c Define 
        complex (fp_kind), dimension(:,:), allocatable ::      A
        complex (fp_kind), dimension(:), allocatable ::      TAU, WORK
        real (fp_kind), dimension(:), allocatable ::      RWORK
        integer, dimension(:), allocatable ::      IWORK, JPIV
        real (8) ::      time_start,time_end
        real (8) :: omp_get_wtime
        real (8) :: abstol, reltol, ABSMAXC2NRMK, RELMAXC2NRMK
...
ccc        write(*,*) lwork
        ...
        allocate(work(100))
        LWORK = -1
        call ZGEQP3RK( M, N, NRHS, KMAX, ABSTOL, RELTOL, A, LDA,
